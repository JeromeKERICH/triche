...
export const StrategyPackageModal = ({ pkg, onClose }) => {
  ...
  }, [pkg.name, pkg.price, paystackReady]);
  ...
  // Calculate KSH amount for display
  const calculateKshAmount = (usdAmount) => {
  ...
  const renderPaymentButton = () => {
    if (!paystackReady) {
      return (
        <button
          disabled
          className={`px-4 py-2 border border-transparent rounded-md text-sm font-medium text-white bg-gray-400 cursor-not-allowed`}
        >
        ...
        return (
          <div className="p-6 text-center">
            ...
            {!paymentInitialized && !paystackReady && (
              <p className="text-sm text-red-500 mt-2">Payment system is still loading. Please wait...</p>
            )}
          </div>
        );
